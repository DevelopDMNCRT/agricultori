<script setup>
import AppHeader from './components/AppHeader.vue'
import AppFooter from './components/AppFooter.vue'
</script>

<template>
  <div class="app-wrapper">
    <AppHeader />
    <main>
      <router-view />
    </main>
    <AppFooter />

    <!-- ── Floating WhatsApp Button ────────────────── -->
    <a
      href="https://wa.me/5213330070631"
      target="_blank"
      rel="noopener noreferrer"
      class="wa-fab"
      id="whatsapp-fab"
      aria-label="Contáctanos por WhatsApp"
    >
      <!-- Pulse ring -->
      <span class="wa-pulse" aria-hidden="true"></span>

      <!-- WhatsApp SVG icon -->
      <svg class="wa-icon" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
      </svg>

      <!-- Label -->
      <span class="wa-label">Contáctanos</span>
    </a>
  </div>
</template>

<style>
/* Global resets or container overrides can be written here,
   but most styles are handled by style.css or locally scoped. */
.app-wrapper {
  display: flex;
  flex-direction: column;
  min-height: 100vh;
}

/* ── WhatsApp FAB ──────────────────────────────────── */
.wa-fab {
  position: fixed;
  bottom: 2rem;
  right: 2rem;
  z-index: 9000;
  display: flex;
  align-items: center;
  gap: 0.6rem;
  background: #25D366;
  color: #ffffff;
  border-radius: 999px;
  padding: 0.7rem 1.2rem 0.7rem 0.85rem;
  box-shadow:
    0 4px 12px rgba(37, 211, 102, 0.35),
    0 8px 30px rgba(37, 211, 102, 0.2);
  text-decoration: none;
  font-family: var(--font-title);
  font-size: 0.9rem;
  font-weight: 700;
  letter-spacing: 0.2px;
  white-space: nowrap;
  transition: transform 0.35s cubic-bezier(0.34, 1.56, 0.64, 1),
              box-shadow 0.35s ease,
              background 0.25s ease;
  /* Slide-up entrance */
  animation: wa-enter 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) 1.2s both;
}

.wa-fab:hover {
  transform: translateY(-4px) scale(1.04);
  background: #1ebe5d;
  box-shadow:
    0 8px 20px rgba(37, 211, 102, 0.45),
    0 16px 40px rgba(37, 211, 102, 0.25);
}

.wa-fab:active {
  transform: scale(0.97);
}

@keyframes wa-enter {
  from {
    opacity: 0;
    transform: translateY(24px) scale(0.85);
  }
  to {
    opacity: 1;
    transform: translateY(0) scale(1);
  }
}

/* Pulse ring */
.wa-pulse {
  position: absolute;
  inset: 0;
  border-radius: 999px;
  border: 2px solid rgba(37, 211, 102, 0.6);
  animation: wa-pulse-ring 2.5s ease-out infinite;
  pointer-events: none;
}

@keyframes wa-pulse-ring {
  0%   { transform: scale(1);   opacity: 0.7; }
  70%  { transform: scale(1.22); opacity: 0; }
  100% { transform: scale(1.22); opacity: 0; }
}

/* Icon */
.wa-icon {
  width: 22px;
  height: 22px;
  flex-shrink: 0;
  position: relative;
  z-index: 1;
}

/* Label */
.wa-label {
  position: relative;
  z-index: 1;
  line-height: 1;
}

/* Mobile: colapsar a solo ícono */
@media (max-width: 480px) {
  .wa-fab {
    padding: 0.85rem;
    border-radius: 50%;
    bottom: 1.5rem;
    right: 1.5rem;
  }

  .wa-label {
    display: none;
  }
}
</style>
